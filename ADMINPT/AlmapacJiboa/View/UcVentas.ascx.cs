...
{
    public partial class UcVentas : System.Web.UI.UserControl
    {
        //DateTime dt_fecha.Value;
        ...
        private void Inicio()
        {
            UCEncabezado.Titulo = "VENTA AL EXTERIOR";
            UCBarraNavegacion.btn_nuevo.Visible = false;
            UCBarraNavegacion.btn_guardar.Visible = true;
            UCBarraNavegacion.btn_cancelar.Visible = true;
            Nuevo();
        ...
        public void dtCliente()
        {
            try
            {
                ...
                Conn.cn.Open();
                DataTable Dt;
                Dt = CDats._SEL_CLIENTE_EXPORTACION(cb_cliente.Value);
                if (Dt.Rows.Count > 0)
                {
                    DataRow row = Dt.Rows[0];

                    txtNit.Text = Convert.ToString(row["NIT_DUI"].ToString());
        ...
        private void InsEnc()
        {
            ...
            try
            {
                ...
                Conn.cn.Close();
                Conn.cn.Open();
                SqlCommand guardar = new SqlCommand("CRE_ENTRADA_SALIDA_ENCA_VT_ALMAPAC_EXP", Conn.cn);
                ...
                guardar.Parameters.AddWithValue("@ENCNRC", DBNull.Value);
                guardar.Parameters.AddWithValue("@ENCDEPARTAMENTO", DBNull.Value);
                guardar.Parameters.AddWithValue("@ENCMUNICIPIO", DBNull.Value);
                guardar.Parameters.AddWithValue("@ENCGIRO", DBNull.Value);
                ...
                guardar.Parameters.AddWithValue("@ID_CLIENTEEXP", Convert.ToInt32(cb_cliente.Value));
