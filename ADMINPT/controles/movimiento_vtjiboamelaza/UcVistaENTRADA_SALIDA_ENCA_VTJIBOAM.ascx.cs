...
{
    public partial class UcVistaENTRADA_SALIDA_ENCA_VTJIBOAM : System.Web.UI.UserControl
    {
        #region "Propiedades"
        ...
        private void ModoEdicion(Boolean esEdicion)
        {
            ...
            txtCliente.ClientEnabled = esEdicion;
            txtNit.ClientEnabled = esEdicion;
            txtNrc.ClientEnabled = esEdicion;
            txtDepartamento.ClientEnabled = esEdicion;
            txtMunicipio.ClientEnabled = esEdicion;
            txtGiro.ClientEnabled = esEdicion;
        ...
        public string ActualizarVtaJiboa()
        {
            ...
            Entidad.ID_FMPAGO = 0;
            Entidad.EFECTIVO = Convert.ToDouble(0);
            Entidad.CHEQUE = Convert.ToDouble(0);
            Entidad.NOTAABONO = Convert.ToDouble(0);
            Entidad.TOTAL = Convert.ToDouble(0);
            Entidad.NCUENTA = Convert.ToString(DBNull.Value);
            Entidad.NCHEQUE = Convert.ToString(DBNull.Value);
            Entidad.BANCO = Convert.ToString(DBNull.Value);
                Entidad.MCHEQUE = Convert.ToDouble(0);
                Entidad.MNOTAABONO = Convert.ToDouble(0);
                //detalle
        ...
        private void Nuevo()
        {
            ...
            UcListaENTRADA_SALIDA_DETA_VTJIBOAM._gridLista.DataSource = DT;
            UcListaENTRADA_SALIDA_DETA_VTJIBOAM._gridLista.DataBind();
            ...
            txtCliente.ClientEnabled = true;
            txtNit.ClientEnabled = true;
            txtNrc.ClientEnabled = true;
            txtDepartamento.ClientEnabled = true;
            txtMunicipio.ClientEnabled = true;
            txtGiro.ClientEnabled = true;
            txtDireccion.ClientEnabled = true;
            FechaDoc();
...
#pragma warning restore CS0168 // La variable 'ex' se ha declarado pero nunca se usa
        ...
        private void EncaFact()
        {
            try
            {
                ...
                Conn.cnph.Close();
                Conn.cnph.Open();
                System.Data.OleDb.OleDbCommand Comando = new System.Data.OleDb.OleDbCommand("select c.nombrecli,c.direcli,c.registro,c.nit, d.nom_deptog, mm.nombre as municipio " +
                ...
                {
                    DataRow row = Dt.Rows[0];
                    txtCliente.Text = Convert.ToString(row["nombrecli"].ToString());
                    txtDireccion.Text = Convert.ToString(row["direcli"].ToString());
                    txtNit.Text = Convert.ToString(row["nit"].ToString());
                    txtNrc.Text = Convert.ToString(row["registro"].ToString());
                    txtDepartamento.Text = Convert.ToString(row["nom_deptog"].ToString());
                    txtMunicipio.Text = Convert.ToString(row["municipio"].ToString());
                }
                else
                {
                    lbl_mensaje.Text = "";
        ...
        private void DetFact()
        {
            try
            {
                ...
                System.Data.OleDb.OleDbCommand Comando = new System.Data.OleDb.OleDbCommand("select  b.codigoart as codigoart, b.cantidad  from facturas a, detafac b " +
                     " where a.tipofact=b.tipofact and VAL(a.NUMERO)=VAL(b.NUMERO) and val(b.codigoart)<>9920 and" +
                    " SUBS(a.tipofact,1,2)=='" + Convert.ToString(cbxTipoDocument.SelectedItem.Value) + "' and  val(a.Numero)= " + txt_ndocument.Text + "  and a.fecha={" + SALF + "}", Conn.cnph);
        ...
        protected void btdui_Click(object sender, EventArgs e)
        {
            if(Convert.ToString(txtdui.Text)!="")
            {
                try
                {
                    ...
                    System.Data.OleDb.OleDbCommand Comando = new System.Data.OleDb.OleDbCommand("select c.nombrecli  , c.direcli, c.registro, c.nit, d.nom_deptog, mm.nombre as municipio " +
                    " from facturas a, clientes c, deptog d, municipi mm " +
                    " where a.cliente=c.cliente and d.deptog=c.depto and mm.codmuni=c.codmuni and  ltrim(rtrim(c.Cliente))='" + txtdui.Text + "'", Conn.cnph);
                    Comando.CommandType = CommandType.Text;
                    Dadapter = new OleDbDataAdapter(Comando);
                    Dadapter.Fill(Dt);
                    if (Dt.Rows.Count > 0)
        ...
        {
            if (Convert.ToString(txtNrc.Text) != "")
            {
                try
                {
                    ...
                    System.Data.OleDb.OleDbCommand Comando = new System.Data.OleDb.OleDbCommand("select c.nombrecli,c.direcli,c.registro,c.nit, d.nom_deptog, mm.nombre as municipio " +
                    " from facturas a, clientes c, deptog d, municipi mm " +
                    " where a.cliente=c.cliente and d.deptog=c.depto and mm.codmuni=c.codmuni and ltrim(rtrim(c.Cliente))='" + txtNrc.Text + "'", Conn.cnph);
                    Comando.CommandType = CommandType.Text;
                    Dadapter = new OleDbDataAdapter(Comando);
                    Dadapter.Fill(Dt);
                    if (Dt.Rows.Count > 0)
