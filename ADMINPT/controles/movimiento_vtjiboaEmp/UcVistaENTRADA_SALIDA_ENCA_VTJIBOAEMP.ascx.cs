...
{
    public partial class UcVistaENTRADA_SALIDA_ENCA_VTJIBOAEMP : System.Web.UI.UserControl
    {
        #region "Propiedades"
        ...
        public string ActualizarVtaJiboa()
        {
            ...
            foreach (DataRow row in DT.Rows)
            {
                ...
                EntidadDt.ID_PRODUCTO = Convert.ToInt32(row["ID_PRODUCTO"]);
                EntidadDt.ID_PRESEN_TRAS = Convert.ToInt32(row["ID_PRESEN_TRAS"]);
                EntidadDt.ID_UNIDAD_FAC = Convert.ToInt32(row["ID_UNIDAD_FAC"]);
                EntidadDt.CANTIDAD = Convert.ToSingle(row["CANTIDAD"]);
                EntidadDt.FACTOR = Convert.ToDouble(row["FACTOR"]);
                EntidadDt.REFERENCIA_DETA = Guid.NewGuid();
                EntidadDt.FACTORKG = Convert.ToDouble(50);
                EntidadDt.ID_BODEP = Convert.ToInt32(Session["ID_BODEP"]);
                ControladorDt.AgregarVtaJiboa(EntidadDt);
            }
        ...
        private void Nuevo()
        {
            ...
            dteFecha.Value = Convert.ToDateTime("01/10/2021"); //DateTime.Now.ToString();
        ...
        private void DetFact()
        {
            try
            {
                ...
                Conn.cnph.Close();
                Conn.cnph.Open();
                System.Data.OleDb.OleDbCommand Comando = new System.Data.OleDb.OleDbCommand("select  IIF(b.codigoart = '10001           ', '9               ', IIF(b.codigoart = '10002           ', '10              ',IIF(b.codigoart = '10003           ', '11              ', '0               ')))  as codigoart, b.cantidad  from facturas a, detafac b " +
                ...
                {
                    //DataRow row = Dt.Rows[0];
                    //txtGiro.Text = Convert.ToString(row["codigoart"].ToString());
                    ...
                    speCantidad.Value = Convert.ToInt32(speCantidad.Value) + Convert.ToInt32(row["cantidad"]);


                    DT.Rows.Add(null, Convert.ToInt32(row["codigoart"]) , 2, 1, Convert.ToDouble(row["cantidad"].ToString()), 1.087, Guid.NewGuid());
