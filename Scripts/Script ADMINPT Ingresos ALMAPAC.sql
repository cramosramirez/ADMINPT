...
SELECT 
	ROW_NUMBER() OVER(ORDER BY D.FECHA) + (SELECT ISNULL(MAX(D.ID_ES_DETA),0) FROM ENTRADA_SALIDA_DETA D) AS ID_ES_DETA,
	E.ID_ES_ENCA,
	D.FECHA, D.NUM_DOC, D.ID_PRODUCTO, D.ID_PRESEN_TRAS, D.ID_UNIDAD_FAC, ABS(CAST(E.OBSERVACION AS NUMERIC(20,4))) * (100 / 2.174) AS CANTIDAD, D.FACTOR,
	NEWID() AS REFERENCIA_DETA, 
	'DBO_ADMINPT_CRUDA_DIF' AS USUARIO_CREA, GETDATE() AS FECHA_CREA, 'DBO_ADMINPT_CRUDA_DIF' AS USUARIO_ACT, GETDATE() AS FECHA_ACT,
...
DELETE FROM ENTRADA_SALIDA_DETA WHERE USUARIO_CREA IN('DBO_ADMINPT_CRUDA','DBO_ADMINPT_CRUDA_DIF','DBO_ADMINPT_MELAZA','DBO_ADMINPT_ENSACADO','DBO_ADMINPT_MELAZA_RECONV','DBO_ADMINPT')
DELETE FROM ENTRADA_SALIDA_ENCA WHERE USUARIO_CREA IN('DBO_ADMINPT_CRUDA','DBO_ADMINPT_CRUDA_DIF','DBO_ADMINPT_MELAZA','DBO_ADMINPT_ENSACADO','DBO_ADMINPT_MELAZA_RECONV','DBO_ADMINPT')
