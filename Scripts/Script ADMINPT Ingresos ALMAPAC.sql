...
SELECT 
	...
	'DBO_ADMINPT_CRUDA_ENSACADA_RECONV' AS USUARIO_CREA, GETDATE() AS FECHA_CREA, 'DBO_ADMINPT_CRUDA_ENSACADA_RECONV' AS USUARIO_ACT, GETDATE() AS FECHA_ACT,
	D.FACTORKG, D.QUINTALES, D.KILOGRAMOS, D.GALONES, D.TM, D.TM_JIBOA, D.TM_ALMAPAC, D.TM_DIF_ALMAPAC, D.TM_DIF_JIBOA, D.FACTORTM, D.PESOKG, D.ID_BODEP
...
SELECT 
	ROW_NUMBER() OVER(ORDER BY E.ID_ES_ENCA) + (SELECT ISNULL(MAX(E.ID_ES_ENCA),0) FROM ENTRADA_SALIDA_ENCA E) AS ID_ES_ENCA,
	ROW_NUMBER() OVER(ORDER BY E.ID_ES_ENCA) + (SELECT ISNULL(MAX(E.ID_ES_ENCA),0) FROM ENTRADA_SALIDA_ENCA E) AS PARENT_ID, E.COD_CAPTURA, 
	CASE
